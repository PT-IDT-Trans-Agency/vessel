...
<html lang="en">
  <head>    
        ...
        $(document).ready(function(){
            ...
            function IsValidTextField(){
                ...
                if ($("#shipper_name").val() == "" || $("#shipper_name").val() == null) {
                    ...
                    $("#invalid-feedback-shipper-name").append("This Shipper Name field is required")
            ...
            $("#btnSimpan").click(function(){
                if (!IsValidTextField()){
                    ...
                    $.ajax({
                        method :"post",
                        url : "<?= base_url("apishipper/update_data_shipper") ?>",
                        dataType:"json",
                        data : data,
                        async : false,
                        headers: {
    ...
    <script type="text/javascript">
        ...
        function SetDataViewShipper(){
            $('#myModalSpinnerPleaseWait').modal({backdrop: 'static', keyboard: false});
            table_shipper.destroy();
            table_shipper = $("#table-shipper").DataTable({
                "ajax" : {
                ...
                "columns" : [
                    {data :  null,
                        render : (data) =>{
                        return '<input type="checkbox" class="form-control-sm selected-row" value="'+data.shipper_code+'" onchange="OnChangeSelectedRow()">';
                        }
                    },
                    {data :  null,
                        render : (data) =>{
                    ...
                    {data :  null,
                        render : (data) =>{
                            id = data.shipper_code
                            param = data.shipper_code+";"+data.shipper_name
                            ...
                            html += "<button class='dropdown-item' onclick='changesShipperName(\""+param+"\")'><i class='fas fa-edit text-primary'></i> <span class='mx-2 text-primary'>Edit Data</span></button>";
                            html += "<button class='dropdown-item' onclick='deleteShipper("+id+")'><i class='fas fa-times text-danger'></i> <span class='mx-2 text-danger'>Hapus Data</span></div>";
                            html += '</div>'
                            html += '</div>'
                            return html
                        }
        ...
        function ClearFormShipper(){
            $("#invalid-feedback-shipper-name").empty();
            $("#shipper_name").removeClass("form-control is-invalid").addClass("form-control");
            $("#shipper_name").removeClass("form-control is-valid").addClass("form-control");
            $("#shipper_name").val("");
            $("#shipper_code").val("");
        }
        function changesShipperName(params){
            ClearFormShipper();
            var datas = params.split(";");
            $("#shipper_code").val(datas[0]);
            $("#shipper_name").val(datas[1]);
            $("#modalFormShipper").modal("show");
            $("#shipper_name").focus();
        }
        function deleteShipper(id){
            swal({
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        method :"post",
                        url : "<?= base_url("apishipper/delete_data_shipper/") ?>",
                        dataType:"json",
                        data : data,
                        async : false,
                        headers: {
  ...
  <body class="c-app">
    ...
    <div class="c-wrapper c-fixed-components">
      ...
      <div class="c-body">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="d-flex justify-content-between">
                            <div>
                                <h4 class="card-title mb-0 text-primary">Shipper</h4>
                            </div>
                            <div >
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <div class="table-responsive">
                                                <table class="table table-striped " id="table-shipper">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th class="bg-primary text-white">Shipper Name</th>
        ...
        <div class="modal" id="modalFormShipper" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable  modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary">
                        <h5 class="text-white"><span class="fas fa-edit"></span> <span class="mx-2">Edit Shipper</span></h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="shipper_name" class="control-label">Shipper Name</label>
    ...
    <script>
        var table_shipper = $("#table-shipper").DataTable();
    </script>
